...
export const DemoFrameWrapper: React.FC<DemoFrameWrapperProps> = ({ demo, children }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col antialiased">
      {/* Top DigitalFlowHub Demo Control Bar */}
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-3">
            <Link
              ...
              title="Return to DigitalFlowHub Demo Gallery"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">Back to Demo Gallery</span>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-medium text-slate-300">
                <span className="text-indigo-400 font-semibold font-heading">DigitalFlowHub Demo:</span>{' '}
                <span className="text-slate-100 font-medium hidden md:inline">{demo.name}</span>
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-indigo-600 via-indigo-500 to-cyan-500 hover:from-indigo-500 hover:to-cyan-400 text-white text-xs font-semibold shadow-lg shadow-indigo-600/30 transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
      ...
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 w-[95%] max-w-2xl bg-slate-900/90 backdrop-blur-xl border border-indigo-500/30 rounded-2xl p-3 shadow-2xl shadow-black/80 flex items-center justify-between gap-3 animate-in slide-in-from-bottom-5 duration-300">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-semibold text-white">
              Like this <span className="text-cyan-400 font-bold">{demo.name}</span> concept?
            </p>
            <p className="text-[11px] text-slate-400 hidden sm:block">
              We customize and launch this exact design for your brand in 5–7 days.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            to={`/contact?demo=${demo.slug}`}
            className="px-3.5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs whitespace-nowrap shadow-md shadow-indigo-600/30 flex items-center gap-1.5 transition-all"
          >
            <span>Customize for My Brand</span>
            ...
            <div className="space-y-4">
              <div>
                <span className="text-[11px] font-mono text-indigo-400 uppercase tracking-wider font-semibold">
                  {demo.categoryLabel} Demo Specification
                </span>
