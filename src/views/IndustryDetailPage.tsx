...
export const IndustryDetailPage: React.FC<{ slug?: string }> = ({ slug }) => {
  ...
  const industrySlug = slug || (params?.industrySlug as string);
  ...
  if (!solution) {
    return (
      <div className="pt-36 pb-24 text-center text-white space-y-4">
        <h2 className="text-2xl font-bold">Industry Not Found</h2>
        <Link href="/solutions" className="text-cyan-400 underline text-sm">
      ...
      {matchingDemo && (
        <div className="p-8 sm:p-12 rounded-3xl bg-slate-900/90 border border-slate-800 space-y-8 shadow-2xl">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
            ...
            <Link
              href={matchingDemo.route}
              className="px-6 py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-indigo-600/30 flex items-center gap-2 self-start md:self-auto transition-all"
            >
